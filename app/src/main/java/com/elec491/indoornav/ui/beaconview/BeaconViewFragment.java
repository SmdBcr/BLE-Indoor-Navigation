package com.elec491.indoornav.ui.beaconview;
...
public abstract class BeaconViewFragment extends Fragment {
    ...
    protected IBeaconFilter uuidFilter = new IBeaconFilter(IndoorPositioningAdvertisingPacket.INDOOR_POSITIONING_UUID, UUID.fromString("74278bda-b644-4520-8f0c-720eaf059935"));
...
/*
        ...
        Beacon bb = beaconFactory.createBeacon(advertisingPacket);
*/

        List<Beacon> beacons = new ArrayList<>();
        for (Beacon beacon : beaconManager.getBeaconMap().values()) {
            Log.i("TEST", beacon.toString());
            for (BeaconFilter beaconFilter : beaconFilters) {
                if (beaconFilter.matches(beacon)) {
                   ...
                   /* if (beacon.getMacAddress().equalsIgnoreCase("D4:36:39:BB:E5:01")){
                        bb.addAdvertisingPacket(beacon.getLatestAdvertisingPacket());
                        bb.setMacAddress("D4:36:39:BB:E5:02");
                        Log.i("TTTTTTTTTT", "entered if");
                        Log.i("TTTTTTTTTT", bb.toString());
